...
export const changePassword = async (userId: string, passwordData: ChangePasswordRequest): Promise<UserOperationResult> => {
  ...
  // Verify current password using Bun's built-in function
  const isCurrentPasswordValid = await Bun.password.verify(passwordData.currentPassword, currentUser[0].password);
  ...
  // Hash new password using Bun's built-in function
  const hashedNewPassword = await Bun.password.hash(passwordData.newPassword);
...
export const addToCart = async (userId: string, cartData: AddToCartRequest): Promise<CartOperationResult> => {
  ...
  const product = await db
    ...
    .from(products)
    .where(eq(products.id, cartData.productId))
    .limit(1);
  ...
  // Check inventory
  if ((product[0].inventoryQuantity ?? 0) < cartData.quantity && !product[0].allowBackorder) {
    throw createValidationError([
      {
        field: "quantity",
        message: `Only ${product[0].inventoryQuantity ?? 0} items available`,
      },
    ]);
  }
  ...
  if (existingItem[0]) {
    ...
    if ((product[0].inventoryQuantity ?? 0) < newQuantity && !product[0].allowBackorder) {
      throw createValidationError([
        {
          field: "quantity",
          message: `Cannot add ${cartData.quantity} more items. Only ${(product[0].inventoryQuantity ?? 0) - existingItem[0].quantity} items can be added`,
        },
      ]);
    }
...
export const updateCartItem = async (
  ...
  if ((product[0].inventoryQuantity ?? 0) < updateData.quantity && !product[0].allowBackorder) {
    throw createValidationError([
      {
        field: "quantity",
        message: `Only ${product[0].inventoryQuantity ?? 0} items available`,
